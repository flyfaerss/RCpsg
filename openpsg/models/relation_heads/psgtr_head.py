...
import torchvision
from mmcv.cnn import Conv2d, Linear, build_activation_layer, constant_init
...
class PSGTrHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        r_class_weight = self.weight_ceiling
        r_class_weight[0] = 0.01
    ...
    '''def _init_layers(self):
        self.query_embed = nn.Embedding(self.num_query, 2 * self.embed_dims)
    ...
    def forward(self, mlvl_feats, img_metas=None):
        ...
        inter_s_references, inter_o_references = self.transformer(
            mlvl_feats,
            mlvl_masks,
            self.query_embed.weight,
            mlvl_positional_encodings,
            r_reg_branches=self.r_reg_branches,  # noqa:E501
            s_reg_branches=None, # self.s_reg_branches,
            o_reg_branches=None, # self.o_reg_branches,
        )
        ...
        # memory = memory[:, -len_last_feat:, :].view(batch_size, self.embed_dims, hw_lvl[-1][0], hw_lvl[-1][1])


        hs = hs.permute(0, 2, 1, 3)
        r_outputs_classes, s_outputs_classes, o_outputs_classes = [], [], []
        s_outputs_coords, o_outputs_coords = [], []
        ...
        for lvl in range(hs.shape[0]):
            ...
            s_tmp = self.r_reg_branches[lvl](hs[lvl]) + self.s_reg_branches[lvl](hs[lvl])
            o_tmp = self.r_reg_branches[lvl](hs[lvl]) + self.o_reg_branches[lvl](hs[lvl])
    ...
    def _get_bboxes_single(self,
        ...
        if self.use_mask:
            ...
            s_mask_pred = s_mask_pred > 0.5 # torch.sigmoid(s_mask_pred) > 0.85
            o_mask_pred = o_mask_pred > 0.5 # torch.sigmoid(o_mask_pred) > 0.85
            ...
            keep = (s_labels != (s_logits.shape[-1] - 1)) & (
                    ...
                    s_scores[keep_tri]>0.4) & (o_scores[keep_tri] > 0.4) & (r_scores > 0.3) ## the threshold is set to 0.85
