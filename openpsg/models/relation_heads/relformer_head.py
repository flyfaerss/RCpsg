...
class RelFormerHead(RelationHead):
    def __init__(self, **kwargs):
        super(RelFormerHead, self).__init__(**kwargs)
    ...
    def forward(self,
        ...
        '''new_roi_feats, union_feats, det_result = self.prepocess_features(
            img, img_meta, det_result, gt_result
        )'''
        ...
        if self.use_bias:
            '''rel_scores = rel_scores + self.freq_bias.index_with_labels(
                pair_pred.long())'''
            rel_scores = combine_rel_scores + self.freq_bias.index_with_labels(
                pair_pred.long())
