...
class RelationSampler(object):
    ...
    def premask_relsample(self, det_result, gt_result):
        pred_labels, target_labels, target_inds = det_result.labels, det_result.target_labels, det_result.target_inds
        gt_labels, gt_rels = gt_result.labels, gt_result.rels
        device = pred_labels[0].device

        rel_pair_idxes = []
        relmaps = []
        target_rel_labels = []

        for idx, (pred_label, target_label, target_ind, gt_label, gt_rel) in enumerate(
            zip(pred_labels, target_labels, target_inds, gt_labels, gt_rels)
        ):
            num_pred_obj = pred_label.shape[0]
            relmap = torch.zeros((num_pred_obj, num_pred_obj), device=device).long()
            target_ind = target_ind - 1

            all_rel_map = torch.zeros((num_pred_obj, num_pred_obj), device=device) - torch.eye(num_pred_obj, device=device)
            fg_rel_pair_idxs = []
            fg_target_rel_label = []
            for gt_pair in gt_rel:
                if (gt_pair[0] in target_ind) and (gt_pair[1] in target_ind):
                    sub_target_index = torch.nonzero(target_ind == gt_pair[0]).flatten()[0]
                    obj_target_index = torch.nonzero(target_ind == gt_pair[1]).flatten()[0]
                    # assert len(sub_target_index) == 1
                    relmap[sub_target_index, obj_target_index] = 1
                    relmap[obj_target_index, sub_target_index] = 1
                    all_rel_map[sub_target_index, obj_target_index] = 1
                    fg_rel_pair_idxs.append([sub_target_index, obj_target_index])
                    fg_target_rel_label.append(gt_pair[2])

            fg_rel_pair_idxs = torch.tensor(fg_rel_pair_idxs, device=device).long()
            fg_target_rel_label = torch.tensor(fg_target_rel_label, device=device).long()

            bg_rel_pair_idxs = torch.nonzero(all_rel_map == 0)
            num_neg_per_img = min(
                self.num_rel_per_image - fg_rel_pair_idxs.shape[0],
                bg_rel_pair_idxs.shape[0])
            '''num_neg_per_img = min(
                            3 * fg_rel_triplets.shape[0],
                            bg_rel_triplets.shape[0])'''
            if bg_rel_pair_idxs.shape[0] > 0:
                perm = torch.randperm(bg_rel_pair_idxs.shape[0],
                                      device=device)[:num_neg_per_img]
                bg_rel_pair_idxs = bg_rel_pair_idxs[perm]
            bg_target_rel_label = torch.zeros(num_neg_per_img, device=device).long()
            target_rel_label = torch.cat([fg_target_rel_label, bg_target_rel_label], dim=0)
            rel_pair_idx = torch.cat([fg_rel_pair_idxs, bg_rel_pair_idxs], dim=0)

            relmaps.append(relmap)
            target_rel_labels.append(target_rel_label)
            rel_pair_idxes.append(rel_pair_idx)

        return target_rel_labels, rel_pair_idxes, relmaps
