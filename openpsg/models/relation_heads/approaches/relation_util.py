...
class Result(object):
    ...
    def __init__(
        ...
        target_pair_labels=None,
        seg_scores=None,
        resistance_bias=None,
        target_inds=None,
        rel_masks=None,
        target_masks=None,
        target_rel_masks=None,
...
class PostProcessor(nn.Module):
    ...
    def get_neighbor_matrix(self, mask):
        num_obj = mask.shape[0]
        neighbor_matrix = np.zeros((num_obj, num_obj))
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (21, 21))
        img_mask = mask.astype(np.uint8)

        for i in range(num_obj):
            temp_img = cv2.dilate(img_mask[i], kernel=kernel, iterations=1)
            for j in range(i + 1, num_obj):
                temp_flag = np.sum((temp_img & img_mask[j]).reshape(-1)) > 0
                neighbor_matrix[i, j] = temp_flag
                neighbor_matrix[j, i] = temp_flag

        return neighbor_matrix

    def select(self, neighbor_matrix, rel_pair_idx, rel_class, allow_label):
        num_rel = rel_pair_idx.shape[0]
        neighbor_flag = torch.zeros(num_rel, dtype=torch.float32).to(neighbor_matrix.device)
        for i in range(num_rel):
            a = (not neighbor_matrix[rel_pair_idx[i, 0], rel_pair_idx[i, 1]])
            b = rel_class
            if not neighbor_matrix[rel_pair_idx[i, 0], rel_pair_idx[i, 1]] and rel_class[i] not in allow_label:
                neighbor_flag[i] = 0.2
        return neighbor_flag

    def forward(self, det_result, key_first=False, freq_matrix=None):
        """
        Arguments:
            det_result
        ...
        # freq_matrix = freq_matrix[1:, 1:, 1:].to(relation_logits[0])
        # freq_matrix = torch.clip(F.softmax(freq_matrix, -1), max=0.2)

        for i, (rel_logit, obj_logit, rel_pair_idx, bbox, mask) in enumerate(
                zip(relation_logits, finetune_obj_logits, rel_pair_idxes,
                    det_result.bboxes, masks)):
            ...
            temp = torch.ones_like(obj_scores)
            ...
            if bbox.shape[1] == 4:
                ...
                bbox = torch.cat((bbox, temp[:, None]), dim=-1)
            else:
                # bbox[:, -1] = obj_scores
                bbox[:, -1] = 1
            finetune_bboxes.append(bbox) # a keng
            ...
            # sub_label = obj_class[rel_pair_idx[:, 0]]
            # obj_label = obj_class[rel_pair_idx[:, 1]]
            # temp_1 = rel_class_prob.cpu().numpy()
            # temp = freq_matrix[obj_pred[rel_pair_idx[:, 0]] - 1, obj_pred[rel_pair_idx[:, 1]] - 1, :]
            # rel_class_prob = rel_class_prob[:, 1:] + freq_matrix[obj_pred[rel_pair_idx[:, 0]] - 1, obj_pred[rel_pair_idx[:, 1]] - 1, :]
            # rel_scores, rel_class = rel_class_prob.max(dim=1)

            allow_remove_predicate = [1, 2, 3, 9, 18, 24, 25, 27, 34, 43, 50, 55]
            hard_samples = [10, 12, 13, 15, 16, 17, 18, 19, 22, 24, 25, 27, 29, 30, 32, 33, 34, 37, 38, 43, 44, 47, 52, 55]
            # very_hard_samples = [34, 31, 25, 8, 11,  26, 29, 32, 41, 53, 36, 42, 54]

            head_samples = [4, 3, 1, 6, 15, 2, 22, 5]
            body_samples = [12, 48, 49, 17, 24, 7, 21, 23, 47, 37, 16, 27, 13, 38, 18, 56, 52, 55, 46, 50]
            tail_samples = [8, 9, 10, 11, 14, 19, 20, 25, 26, 28, 29, 30, 31, 32, 33, 34, 35, 36, 39, 40, 41, 42, 43, 44, 45, 51, 53, 54]

            # rel_class_prob[:, hard_samples] = rel_class_prob[:, hard_samples] * 2
            # rel_class_prob[:, very_hard_samples] = rel_class_prob[:, very_hard_samples] * 5
            # if rel_class_prob

            rel_scores, rel_class = rel_class_prob[:, 1:].max(dim=1)
            rel_class = rel_class + 1
            
            prior_scores = (freq_matrix[obj_class[rel_pair_idx[:, 0]], obj_class[rel_pair_idx[:, 1]], rel_class] > 0) * 0.1
            prior_scores = prior_scores.to(obj_scores)
            # print(prior_scores)

            '''rel_class_prob_copy = rel_class_prob.clone()
            rel_class_prob_copy[:, rel_class] = 0.0
            second_rel_scores, second_rel_class = rel_class_prob_copy[:, 1:].max(dim=1)
            second_rel_class = second_rel_class + 1
            # print(len(second_rel_class))
            for i in range(len(second_rel_class)):
                if ((second_rel_class[i] in hard_samples) and (rel_class[i] in head_samples)): #  and (2.0 * second_rel_scores[i] > rel_scores[i]):
                    # print(second_rel_class[i], rel_class[i]) # rel_class[i] = second_rel_class[i]
                    temp = rel_logit[i, rel_class[i]].clone()
                    rel_logit[i, rel_class[i]] = rel_logit[i, second_rel_class[i]]
                    rel_logit[i, second_rel_class[i]] = temp
                    # rel_class[i] = second_rel_class[i]
                    temp_1 = rel_class_prob[i, rel_class[i]].clone()
                    rel_class_prob[i, rel_class[i]] = rel_class_prob[i, second_rel_class[i]]
                    rel_class_prob[i, second_rel_class[i]] = temp_1
                    rel_class[i] = second_rel_class[i]'''
            # rel_class_prob[:, rel_class] = 0.0
            # rel_logit[:, rel_class] = -10000.0
            # rel_scores, rel_class = rel_class_prob[:, 1:].max(dim=1)
            # rel_class = rel_class + 1
            # rel_class_prob[:, rel_class] = 0.0
            # rel_logit[:, rel_class] = -10000.0
            ...
            background_scores = rel_class_prob[:, 0]

            # rel_class[rel_class == 1] = 4

            # neighbor_matrix = torch.tensor(self.get_neighbor_matrix(mask)).to(rel_class)
            # neighbor_flag = self.select(neighbor_matrix, rel_pair_idx, rel_class, allow_remove_predicate)
            # rule_flag = self.rule_select(rel_pair_idx, rel_class)
            # TODO Kaihua: how about using weighted some here?  e.g. rel*1 + obj *0.8 + obj*0.8
            triple_scores = rel_scores * obj_scores0 * obj_scores1 # - neighbor_flag # + prior_scores - neighbor_flag # - background_scores # - neighbor_flag
            ...
            prop_rel_scores.append(rel_scores)
