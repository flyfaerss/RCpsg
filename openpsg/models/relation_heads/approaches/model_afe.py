...
class MessageFusion(nn.Module):
    ...
    def forward(self, input, hidden, raw=None):
        if raw is not None:
            ...
            y = self.eps * input + (1 - self.eps) * hidden
            row_norm = torch.norm(y - raw, p=2, dim=1)
            row_norm = torch.clamp(row_norm, min=1e-7)
            beta = torch.clamp(row_norm - self.eps / 2.0, min=0)
            beta = (beta / row_norm).unsqueeze(1)
            output = (1 - beta) * raw + beta * y
