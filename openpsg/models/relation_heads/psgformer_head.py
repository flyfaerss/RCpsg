...
class PSGFormerHead(AnchorFreeHead):
    ...
    def _get_bboxes_single(self,
        ...
        if self.use_mask:
            ...
            output_scores = torch.cat((s_scores, o_scores), 0)
            ...
            final_all_masks = torch.sigmoid(all_masks) > 0.6
            ...
            rel_pairs_index = pan_rel_pairs.clone()
            r_scores = r_scores[no_obj_filter]
            all_rels = torch.cat((rel_pairs_index, r_labels.unsqueeze(-1)), -1)
            final_r_labels, final_r_dists = r_labels.clone(), r_dists.clone()
                    ...
                    for equiv in stuff_equiv_classes.values():
                        if len(equiv) > 1:
                            for eq_id in equiv:
                                ...
                                final_all_masks[equiv[0]].masked_fill_(final_all_masks[eq_id].eq(True), True)
                                rel_pairs_index.masked_fill_(rel_pairs_index.eq(eq_id), equiv[0])
                    ...
                    for equiv in thing_dedup.values():
                        if len(equiv) > 1:
                            for eq_id in equiv:
                                ...
                                final_all_masks[equiv[0]].masked_fill_(final_all_masks[eq_id].eq(True), True)
                                rel_pairs_index.masked_fill_(rel_pairs_index.eq(eq_id), equiv[0])
                    ids_remain,_ = rel_pairs_index.unique().sort()
                    no_obj_filter3 = torch.zeros(pan_rel_pairs.shape[0],dtype=torch.bool)
                    for triplet_id in range(rel_pairs_index.shape[0]):
                        if rel_pairs_index[triplet_id,0] in ids_remain and rel_pairs_index[triplet_id,1] in ids_remain:
                            no_obj_filter3[triplet_id]=True
                    rel_pairs_index = rel_pairs_index[no_obj_filter3]
                    final_r_labels, final_r_dists = final_r_labels[no_obj_filter3], final_r_dists[no_obj_filter3]
                    r_scores = r_scores[no_obj_filter3]

                    temp_mask = [] # final_all_masks.clone()[: temp_len]
                    temp_labels = [] # all_labels.clone()[: temp_len]
                    temp_scores = [] # all_scores.clone()[: temp_len]
                    for i, id_remain in enumerate(ids_remain):
                        temp_scores.append(all_scores[id_remain])
                        temp_labels.append(all_labels[id_remain])
                        temp_mask.append(final_all_masks[id_remain])
                        rel_pairs_index.masked_fill_(rel_pairs_index.eq(id_remain), i)
                    all_rels = torch.cat((rel_pairs_index, r_labels.unsqueeze(-1)), -1)
                    final_all_masks = torch.stack(temp_mask, 0)
                    final_all_labels = torch.stack(temp_labels, 0)
                    all_scores = torch.stack(temp_scores, 0)
        ...
        if self.use_mask:
            return det_bboxes, complete_labels, rel_pairs, output_masks, pan_rel_pairs, \
                ...
                r_scores, all_scores, final_all_labels, final_all_masks, rel_pairs_index, all_rels, final_r_labels, \
                final_r_dists
        else:
