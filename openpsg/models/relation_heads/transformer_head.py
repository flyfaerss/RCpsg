...
from .approaches import TransformerContext, FreqBiasModule
from .relation_head import RelationHead


@HEADS.register_module()
class RelTransformerHead(RelationHead):
    def __init__(self, **kwargs):
        super(RelTransformerHead, self).__init__(**kwargs)
        self.context_layer = TransformerContext(self.head_config, self.obj_classes, self.rel_classes)
    ...
    def forward(self,
        ...
        '''if self.training:
            pair_gt = []
            for pair_idx, obj_gt in zip(det_result.rel_pair_idxes, det_result.target_labels):
                pair_gt.append(
                    torch.stack(
                        (obj_gt[pair_idx[:, 0]], obj_gt[pair_idx[:, 1]]),
                        dim=1))
            pair_gt = torch.cat(pair_gt, dim=0)
        else:
            pair_gt = pair_pred'''
