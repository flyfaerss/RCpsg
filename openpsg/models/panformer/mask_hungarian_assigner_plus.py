...
class MaskHungarianAssigner_PLUS(BaseAssigner):
    ...
    def __init__(self,
                 ...
                 stuff_thing_flag=False):
        self.cls_cost = build_match_cost(cls_cost)
        self.mask_cost = build_match_cost(mask_cost)
        self.dice_cost = build_match_cost(dice_cost)
        self.stuff_thing_flag = stuff_thing_flag
    ...
    def assign(self,
        ...
        if self.stuff_thing_flag:
            pred_labels = torch.argmax(cls_pred, -1)
            pred_labels[pred_labels < 80] = 0
            pred_labels[pred_labels >= 80] = 1
            temp_gt_labels = torch.zeros_like(gt_labels)
            temp_gt_labels[gt_labels >= 80] = 1
            category_cost = (pred_labels.unsqueeze(-1) == temp_gt_labels.unsqueeze(0)).to(cost)
            category_cost = 1.0 - category_cost
            cost += category_cost
