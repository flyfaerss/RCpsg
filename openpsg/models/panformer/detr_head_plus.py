...
class RelDETRHead(
    ...
    def __init__(
            ...
            s_dice_loss=dict(type='DiceLoss', loss_weight=1.0),
            o_dice_loss=dict(type='DiceLoss', loss_weight=1.0),
        ...
        self.bg_cls_weight = 0.01
        self.sync_cls_avg_factor = sync_cls_avg_factor

        s_cls_weight = s_loss_cls.get('class_weight', None)
        o_cls_weight = o_loss_cls.get('class_weight', None)
        assert isinstance(s_cls_weight, float), 'Expected ' \
            'bg_cls_weight to have type float. Found ' \
            f'{type(s_cls_weight)}.'
        s_cls_weight = torch.ones(num_classes + 1) * s_cls_weight
        o_cls_weight = torch.ones(num_classes + 1) * o_cls_weight
        # set background class as the last indice
        s_cls_weight[-1] = self.bg_cls_weight
        o_cls_weight[-1] = self.bg_cls_weight
        s_loss_cls.update({'class_weight': s_cls_weight})
        o_loss_cls.update({'class_weight': o_cls_weight})

        if 'bg_cls_weight' in s_loss_cls:
            s_loss_cls.pop('bg_cls_weight')
            o_loss_cls.pop('bg_cls_weight')
        ...
        self.weight_ceiling[self.predicate_distribution > self.tail_first] = torch.sqrt(
            torch.exp(self.predicate_distribution[
                          self.predicate_distribution > self.tail_first]) / torch.exp(self.tail_first))
        self.weight_ceiling[self.predicate_distribution < self.tail_first] = torch.sqrt(
            torch.exp(self.predicate_distribution[
                          self.predicate_distribution < self.tail_first]) / torch.exp(self.tail_first))
        self.weight_ceiling[self.predicate_distribution < self.body_first] *= torch.sqrt(
            torch.exp(self.predicate_distribution[
                          self.predicate_distribution < self.body_first]) / torch.exp(self.body_first))
        # self.predicate_weight = torch.ones(head_num + body_num + tail_num + 1)

        r_cls_weight = self.weight_ceiling
        r_cls_weight[0] = 0.01
        r_loss_cls.update({'class_weight': r_cls_weight})
        ...
        if train_cfg:
            ...
            assert s_loss_cls['loss_weight'] == assigner['s_cls_cost']['weight'], \
                'The classification weight for loss and matcher should be' \
                'exactly the same.'
            assert o_loss_cls['loss_weight'] == assigner['o_cls_cost']['weight'], \
                'The classification weight for loss and matcher should be' \
                'exactly the same.'
            assert r_loss_cls['loss_weight'] == assigner['r_cls_cost']['weight'], \
                'The classification weight for loss and matcher should be' \
                'exactly the same.'
            assert s_loss_bbox['loss_weight'] == assigner['s_reg_cost'][
                'weight'], 'The regression L1 weight for loss and matcher ' \
                'should be exactly the same.'
            assert o_loss_bbox['loss_weight'] == assigner['o_reg_cost'][
                'weight'], 'The regression L1 weight for loss and matcher ' \
                           'should be exactly the same.'
            assert s_loss_iou['loss_weight'] == assigner['s_iou_cost']['weight'], \
                'The regression iou weight for loss and matcher should be' \
                'exactly the same.'
            assert o_loss_iou['loss_weight'] == assigner['o_iou_cost']['weight'], \
                'The regression iou weight for loss and matcher should be' \
                'exactly the same.'
        ...
        self.s_loss_cls = build_loss(s_loss_cls)
        self.o_loss_cls = build_loss(o_loss_cls)
        self.r_loss_cls = build_loss(r_loss_cls)
        self.s_loss_bbox = build_loss(s_loss_bbox)
        self.o_loss_bbox = build_loss(o_loss_bbox)
        self.s_loss_iou = build_loss(s_loss_iou)
        self.o_loss_iou = build_loss(o_loss_iou)
        if self.use_mask:
            self.s_dice_loss = build_loss(s_dice_loss)
            self.o_dice_loss = build_loss(o_dice_loss)
    ...
    def _init_layers(self):
        ...
        '''self.input_proj = Conv2d(self.in_channels,
                                 self.embed_dims,
                                 kernel_size=1)
        self.fc_cls = Linear(self.embed_dims, self.obj_cls_out_channels)
        # self.rel_fc_cls = Linear(self.embed_dims, self.rel_cls_out_channels)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is RelDETRHead:
            convert_dict = {
