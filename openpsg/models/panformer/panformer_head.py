...
from .detr_head import DETRHeadv2
...
class PanformerHead(DETRHeadv2):
    ...
    def __init__(
        ...
        if datasets == 'coco':
            from .color_map import id_and_category_maps
    ...
    def loss_single_panoptic(self,
        ...
        loss_cls, loss_iou, loss_bbox, pos_inds_mask_list, num_total_pos_thing = self.get_filter_results_and_loss(
            cls_scores, bbox_preds, cls_scores_list, bbox_preds_list, gt_bboxes_list, gt_labels_list, img_metas,
            gt_bboxes_ignore_list)
    ...
    def get_bboxes(
        ...
        for img_id in range(len(img_metas)):
            ...
            thing_query = query_inter_things[-1]
            scores_stuff = self.cls_stuff_branches[-1](
                stuff_query).sigmoid().reshape(-1)

            all_query = torch.cat([thing_query, stuff_query], 1).squeeze(0)

            mask_pred = attn_map.reshape(-1, *hw_lvl[0])

            if peseudo_test:
                input_shape = img_metas[img_id]['batch_input_shape']
                pre_mask_pred = F.interpolate(mask_pred.unsqueeze(0),
                                              size=input_shape,
                                              mode='bilinear').squeeze(0)
                scalar_h, scalar_w = img_shape[:2]
                mask_pred = pre_mask_pred[:, :scalar_h, :scalar_w]
            ...
            pan_results = torch.zeros(*mask_pred.shape[-2:],
                                  device=mask_pred.device).to(torch.long)
            id_unique = 1
            obj_query = []
            rest_query = []
            rest_bbox = []
            rest_labels = []
            use_index = torch.ones(len(scores_all)).bool()
            use_index[scores_all < 0.1] = False
            label2id = {}
            ...
            for i, scores in enumerate(scores_all):
                ...
                pan_results[_mask] = labels_all[i]
                use_index[i] = False
                if labels_all[i] >= 80:
                    label2id[int(labels_all[i])] = i
                if labels_all[i] < 80:
                    pan_results[_mask] = labels_all[i] + id_unique * INSTANCE_OFFSET
                    results[1, _mask] = id_unique
                    label2id[int(labels_all[i] + id_unique * INSTANCE_OFFSET)] = i
                    id_unique += 1
            ...
            for i in range(len(ids)):
                obj_query.append(all_query[label2id[ids[i]]])
            if len(ids) == 0:
                print('\n zero prediction!')
                obj_query = torch.tensor([]).to(thing_query)
            else:
                obj_query = torch.stack(obj_query).to(thing_query)
            ...
            area_list = [np.sum(item) for item in segms]
            height, width = segms.shape[1:]
            masks_to_bboxes = BitmapMasks(segms, height, width).get_bboxes()

            masks_to_bboxes = torch.tensor(masks_to_bboxes).to(bbox)
            ...
            if (self.training and proposal_matching) or (not self.training and peseudo_inference):
                result = dict(pan_results=pan_results,
                              ...
                              use_index=use_index,
                              rest_query=all_query[scores_all > 0.1],
                              rest_bboxes=bboxes_all[scores_all > 0.1][:, :4],
                              rest_masks=masks_all[[scores_all > 0.1]],
                              rest_labels=labels_all[[scores_all > 0.1]] + 1,
                              rest_scores=scores_all[scores_all > 0.1],
                              )
            else:
                result = dict(pan_results=pan_results,
                              masks=segms,
                              ...
                              stuff_flag=stuff_flag,
                              use_index=use_index,
                              rest_query=all_query[use_index],
                              rest_bboxes=bboxes_all[use_index][:, :4],
                              rest_masks=masks_all[use_index],
                              rest_labels=labels_all[use_index],
                              )
            all_results.append(result)

            pan_results = dict(pan_results=pan_results)
            pan_results_list.append(pan_results)
        ...
        return pan_results_list, all_results
