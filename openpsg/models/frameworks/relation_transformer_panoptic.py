...
class RelFormerPanoptic(DETR_plus):
    def __init__(
        self,
        backbone,
        neck=None,
        ...
        super(RelFormerPanoptic, self).__init__(
            backbone=backbone,
            neck=neck,
            bbox_head=bbox_head,
            train_cfg=train_cfg,
    ...
    def simple_test_sg_bboxes(self,
        ...
        pan_results, results = self.bbox_head.get_bboxes(*outs, img_metas,
                            rescale=rescale, peseudo_test=peseudo_test,
                            peseudo_inference=peseudo_inference, proposal_matching=proposal_matching)
    ...
    def detector_simple_test(
        ...
        if not is_testing:  # excluding the testing phase
            ...
            if not self.use_pesuedo_inference:
                ...
                det_bboxes = [torch.tensor(BitmapMasks(pan_seg_masks[0], temp_height, temp_width).get_bboxes()).to(det_results[0]['rest_bboxes'][0])]
