...
class MaskFormerFusionHead(BasePanopticFusionHead):
    ...
    def panoptic_postprocess(self, mask_cls, mask_pred):
        ...
        # seg_score = (cur_masks * seg.float()).sum((1, 2)) / sum_seg
        ...
        if cur_masks.shape[0] == 0:
            ...
            for k in range(cur_classes.shape[0]):
                ...
                if mask_area > 0 and original_area > 0:
                    ...
                    used_idx.append(valid_idx[k])
                    if not isthing:
                        # different stuff regions of same class will be
                        # merged here, and stuff share the instance_id 0.
                        panoptic_seg[mask] = pred_class
                        if label2idx.get(pred_class) is None:
                            label2idx[pred_class] = [int(valid_idx[k])]
                        else:
                            label2idx[pred_class].append(int(valid_idx[k]))
                    else:
                        panoptic_seg[mask] = (
                            pred_class + instance_id * INSTANCE_OFFSET)
                        label2idx[pred_class + instance_id * INSTANCE_OFFSET] = [int(valid_idx[k])]
                        instance_id += 1

        return panoptic_seg, label2idx, used_idx
    ...
    def simple_test(self,
        ...
        for mask_cls_result, mask_pred_result, meta in zip(
            ...
            result = dict()
            if panoptic_on:
                pan_results, label2idx, used_idx = self.panoptic_postprocess(
                    mask_cls_result, mask_pred_result)
                result['pan_results'] = pan_results
                result['label2idx'] = label2idx
                result['used_idx'] = used_idx
