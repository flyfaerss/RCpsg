...
class BaseSemanticHead(BaseModule, metaclass=ABCMeta):
    ...
    def simple_test(self, x, img_metas, rescale=False):
        ...
        w, h = output['feats'].shape[-2:]
        attn = torch.sigmoid(seg_preds)
        # seg_feats = torch.einsum('bnwh,bcwh->bnc', attn, output['feats']) / (w * h)
        seg_feats = torch.einsum('bnwh,bcwh->bncwh', attn, output['feats'])
        # min_v, max_v = torch.min(seg_feats), torch.max(seg_feats)

        '''attn_1 = torch.softmax(seg_preds, dim=1)
        seg_feats_1 = torch.einsum('bnwh,bcwh->bncwh', attn_1, output['feats'])
        seg_feats_1 = F.interpolate(
            seg_feats_1.squeeze(),
            size=(16, 16),
            mode='bilinear',
            align_corners=False).unsqueeze(0)
        seg_feats_1 = torch.sum(seg_feats_1, dim=2)'''


        seg_feats = F.interpolate(
            seg_feats.squeeze(),
            size=(16, 16),
            mode='bilinear',
            align_corners=False).unsqueeze(0)
        seg_feats = torch.sum(seg_feats, dim=2) / 128
        # min_v, max_v = torch.min(seg_feats), torch.max(seg_feats)

        seg_feats = torch.flatten(seg_feats, start_dim=2, end_dim=3)
