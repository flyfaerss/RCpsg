...
class SceneGraphPanopticFPN(PanopticFPN):
    ...
    def simple_test_sg_bboxes(self,
        ...
        for bbox, score in zip(bboxes, scores):
            ...
            # det_scores.append(score[det_inds // 80, det_inds % 80])
        ...
        if mask_results['mask_feats'] is not None:
            ...
            num_obj = [len(masks[i]) if masks[i] is not None else 0 for i in range(len(masks))]

            mask_feats = torch.flatten(torch.sum(mask_feats, dim=1) / 16.0, start_dim=1, end_dim=2)
            mask_feats = mask_feats.split(num_obj, dim=0)
        # min_v, max_v = torch.min(mask_feats), torch.max(mask_feats)
        ...
        for i in range(len(img_metas)):
            seg_pred, seg_feat = self.semantic_head.simple_test(
                [f[i][None, ...] for f in x],
                [img_metas[i]],
                rescale,
            )
            seg_preds.append(seg_pred[0])
            seg_feats.append(seg_feat[0])

        # min_v, max_v = torch.min(seg_preds[0]), torch.max(seg_preds[0])

        # mask_all = [torch.stack((masks, seg_pred), dim=0).sigmoid() for (mask, seg_pred) in zip(masks, seg_preds)]

        results = []
        for i in range(len(det_bboxes)):
            if len(det_bboxes[i]) > 0:
                new_mask = masks[i].clone()
                new_mask[new_mask == 0] = -10000
                scores = torch.cat([det_bboxes[i][:, -1], torch.ones(53).to(bboxes[0])], 0)
                query = torch.cat((mask_feats[i], seg_feats[i][: -1]), dim=0)
                labels_all = torch.cat([det_labels[i], torch.arange(80, 133).to(det_labels[i].device)], 0)
                mask = torch.cat((new_mask, seg_preds[i][: -1]), dim=0).sigmoid()
            else:
                scores = torch.ones(53).to(bboxes[0])
                query = seg_feats[i][: -1]
                labels_all = torch.arange(80, 133).to(scores.device)
                mask = seg_preds[i][: -1].sigmoid()
            seg = mask > 0.6225
            sum_seg = seg.sum((1, 2)).float() + 1

            seg_scores = (mask * seg.float()).sum((1, 2)) / sum_seg
            scores *= seg_scores

            use_idxes = torch.ones(len(scores)).bool()
            use_idxes[scores < 0.5] = False
            ...
            pan_results, label2id = self.panoptic_fusion_head.simple_test(
            ...
            for key, item in label2id.items():
                use_idxes[item] = False

            for j in range(len(ids)):
                obj_query.append(query[label2id[ids[j]]])
            if len(ids) == 0:
                print('\n zero prediction!')
                obj_query = torch.tensor([]).to(bboxes[0])
            else:
                obj_query = torch.stack(obj_query).to(bboxes[0])

            # Extract class labels, (N), 1-index?
            labels = np.array([id % INSTANCE_OFFSET for id in ids],
                              dtype=np.int64) + 1
            stuff_flag = labels >= self.num_things_classes
            # labels = np.array([id % INSTANCE_OFFSET for id in ids],
            # dtype=np.int64)
            ...
            if (self.training and proposal_matching) or (not self.training and peseudo_inference):
                result = dict(pan_results=pan_results,
                            ...
                            use_index=use_idxes,
                            rest_query=query[scores > 0.5],
                            rest_bboxes=None,
                            rest_masks=mask[scores > 0.5],
                            rest_labels=labels_all[scores > 0.5] + 1,
                            rest_scores=scores[scores > 0.5],
                              )
            else:
                result = dict(pan_results=pan_results,
                            masks=segms,
                            ...
                            stuff_flag=stuff_flag,
                            use_index=use_idxes,
                            rest_query=query[use_idxes],
                            rest_bboxes=None,
                            rest_masks=mask[use_idxes],
                            rest_labels=labels_all[use_idxes],
                            )
        ...
        # NOTE: Sgcls should not be performed
            ...
            if not is_testing:  # excluding the testing phase
                ...
                for i in range(len(img_meta)):
                    if self.use_proposal_matching:
                        ...
                        pred_cls = F.one_hot(rest_obj_labels[i] - 1,
                                             num_classes=self.num_classes).to(det_bboxes[0])
                        # print(gt_masks[i].masks, det_bboxes[0].device)
                        # print('aaaaaaaaaaaaaaaaaa')
                        temp_gt_masks = torch.tensor(gt_masks[i].masks).to(rest_obj_masks[i])
                        ...
                        target_masks.append(torch.tensor(gt_masks[i].masks)[temp_inds - 1])
                        det_labels[i] = rest_obj_labels[i][choose_index]
                        temp_height, temp_width = rest_obj_masks[i].shape[1:]
                        rest_obj_masks[i] = rest_obj_masks[i] > 0.6225
                        masks_to_bboxes = BitmapMasks(rest_obj_masks[i][choose_index].detach().cpu().numpy(),
                                                      temp_height,
                        ...
                        target_masks.append(torch.tensor(gt_masks[i].masks)[target_inds[-1] - 1])
                        ...
                        if self.use_full_train_samples and len(pred_cls[i]) < len(gt_labels[i]):
                            ...
                            target_labels[-1] = torch.cat(
                                (target_labels[-1], rest_mask_assign_result.labels[choose_index] + 1))
                            target_inds[-1] = torch.cat(
                                (target_inds[-1], map2init[rest_mask_assign_result.gt_inds[choose_index] - 1] + 1))
                            target_masks[-1] = torch.tensor(gt_masks[i].masks)[target_inds[-1] - 1]
                            det_labels[i] = torch.cat((det_labels[i], rest_obj_labels[i][choose_index] + 1))
                            temp_height, temp_width = rest_obj_masks[i].shape[1:]
                            rest_obj_masks[i] = rest_obj_masks[i] > 0.5
                            masks_to_bboxes = BitmapMasks(rest_obj_masks[i][choose_index].detach().cpu().numpy(),
                                                          temp_height, temp_width).get_bboxes()
                            masks_to_bboxes = torch.tensor(masks_to_bboxes).to(det_bboxes[i])
                ...
                if not self.use_pesuedo_inference:
                    ...
                    det_bboxes = [torch.tensor(BitmapMasks(pan_seg_masks[0], temp_height, temp_width).get_bboxes()).to(
                        det_results[0]['rest_bboxes'][0])]
    ...
    def backward_panoptic_inference(self, det_result):
        ...
        refine_obj_labels = det_result.refine_labels - 1
        ...
        '''instance_cover_num = np.sum(seg_scores.cpu().numpy() > 0.5)
    ...
    def get_cover_mask(self, pan_index, pan_results, init_pan_masks, area_list, inst_index, inst_labels, rel_score,
        ...
        if len(cover_index_list) == 0:
            ...
            if (inst_labels[inst_index - 1] < 80 and cover_ratio < 0.4) or (
                    inst_labels[inst_index - 1] >= 80 and cover_ratio < 0.4):
                _mask = init_pan_masks[inst_index - 1] & (pan_index == 0)
                ...
                for k in range(len(cover_index_list)):
                    if cover_statistic[cover_index_list[k]] / area_list[cover_index_list[k]] > 0.1:
                        allowed_cover = False
                        break
                if allowed_cover and np.sum(init_pan_masks[inst_index - 1]) > 500:
